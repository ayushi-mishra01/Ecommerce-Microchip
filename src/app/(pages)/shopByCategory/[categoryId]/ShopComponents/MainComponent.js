...
import { Link, Typography, Grid, TextField } from '@mui/material'
...
const MainComponent = ({ categoryId }) => {
  ...
  if (searchTerm.trim() === '') {
    ...
    fetchSearchProducts(searchTerm.trim());
  }
  };
  ...
  const fetchProducts = () => {
    axios.get(`${process.env.NEXT_PUBLIC_PUBLISHED_API_URL}/api/Products/getProductsByCategory/${categoryId}`)
      .then(response => {
  ...
  const fetchSearchProducts = async (searchTerm) => {
    try {
      ...
      const response = await axios.get(`${process.env.NEXT_PUBLIC_PUBLISHED_API_URL}/api/Products/getSearchProductsByCategory/${categoryId}?SearchTerm=${encodedSearchTerm}`);
      setProducts(response.data);
    } catch (error) {
      console.error('Error fetching products:', error);
    }
    ...
    <>
      ...
      {/* <TextField
        ...
        {paginatedProducts.map((product, index) => (
          ...
          >
            <img src={product.pictureUrl} alt="productimg" style={{ width: "60%" }} />
            <Typography variant="h7" fontWeight="bold" component="p">
              {product.name}
            </Typography>
            <Typography variant="body2" color="textSecondary">
